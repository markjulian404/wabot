...
const quotemaker = async (quotes, author = 'Julian Official') => {
    var q = quotes.replace(/ /g, '%20').replace('\n','%5Cn')
    const response = await fetch(`http://julianofficial.000webhostapp.com/api/quotegenerator.php?quotes=${q}&wm=${author}`)
    if (!response.ok) throw new Error(`unexpected response ${response.statusText}`)
    const json = await response.json()
    if (json.status) {
        if (json.result !== '') {
