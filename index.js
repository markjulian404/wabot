...
const start = async (client = new Client()) => {
    console.log('[DEV]', color('Julian Official', 'yellow'))
    console.log('[SERVER] Started!')
        ...
        client.onAddedToGroup(((chat) => {
            ...
            if (totalMem < 3) { 
            	client.sendText(chat.id, `anggota group kurang`).then(() => client.leaveGroup(chat.id)).then(() => client.deleteChat(chat.id))
            } else {
    ...
    create('Julian', options(true, start))
